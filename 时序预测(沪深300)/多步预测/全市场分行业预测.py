...
# 训练和测试函数
def train_test(industry, day, args):2333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333
    ##=====数据生成
    # 数据集
    ...
    if need_train:
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                df_to_db(df_out,table_name=table_total)
...
if __name__ == '__main__':
    ...
    for step_, industry in enumerate(industry_df.Industry.to_list()):
        print('*'*60+f'正在训练第{step_+1}个行业:{industry}')
        ...
        table_total = f'{name_}_mn_{model_name}_in_total_st_{start_}_end_{end_}_fs_{features}_ep_{epochs}_bz_{batch_size}_sq_{seq_len}_lb_{label_len}_pd_{pred_len}_tl_{train_len}_sl_{scale}_dm_{d_model}_k_{args.top_k}_nk_{args.num_kernels}'
        ...
        if whether_exist_table(db_file_path = '../daily_stock_daybar/', db_name = 'long_term_forecast', table_name = table_name):
            print('*'*60+f'{industry}行业已训练好在数据库中，无序再进行训练')
            pass
        else:
    ...
    print(f'训练全市场所花费的时间为{(end-start)/3600}小时')
    # 查看全市场最终结果
    order_1(path = f'../回测结果/每日换仓/全市场/', table_name=table_total)
    order_2(path = f'../回测结果/每月换仓/全市场/', table_name=table_total)
    AlphaCheak(f'../回测结果/每日换仓/全市场/')
    AlphaCheak(f'../回测结果/每月换仓/全市场/')
    aggregate(f'../回测结果/每日换仓/全市场/')
    aggregate(f'../回测结果/每月换仓/全市场/')