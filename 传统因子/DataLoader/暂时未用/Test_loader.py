...
class test_dataProcess(Dataset):
    def __init__(self, df_raw, seq_len, label_len, pred_len, pred_times, scale, stock_train_list, mean=None, std=None, timeenc=0, features='MS', freq='h'):
        """
        df_raw: 训练集的原始输入
        pred_times： 训练集训练次数
        scale: 是否需要标准化
        ...
        self.dataframe_len = seq_len + pred_len + pred_times - 1  #回测天数
    ...
    def __read_data__(self):
        ...
        for stock in stock_test_list:
            ...
            if stock not in self.stock_train_list:
                ...
                if df_temp.__len__() - self.seq_len - self.pred_len + 1 <= 0 :
                    continue
                else:
                    df_raw_1 = pd.concat([df_raw_1, df_temp], axis=0)
                    df_len.append(df_temp.__len__() - self.seq_len - self.pred_len + 1) #股票预测次数至少为一次
                    stock_list.append(stock)
        self.df_raw_1 = df_raw_1
        df_stamp = df_raw_1.index.to_frame().reset_index(drop=True)
        self.df_len = df_len
        self.cumsum_pred_times= [sum(self.df_len[:i+1]) for i in range(len(self.df_len))]
    ...
    def __getitem__(self, index):
        # 每只股票训练次数累加
        cumsum_pred_times = [sum(self.df_len[:i+1]) for i in range(len(self.df_len))]
        ...
        def Get_final_index(index):
            if Find_index(index,cumsum_pred_times) == 0:
                a = 0
            else:
                a = cumsum_stockcode_len[Find_index(index,cumsum_pred_times)-1]
            return a + index % cumsum_pred_times[Find_index(index,cumsum_pred_times)-1]
