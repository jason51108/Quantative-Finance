...
def Get_All_Tradingday(engine, date:str, n:int, forward=False)->list:
    ...
    Example:
    >>> Get_All_Tradingday(engine,date='20220101',n=3,forward=False)
...
# 获取给定起始时间和结束时间的交易日期
def Get_All_Tradingday_1(engine, beginday:str, endday:str):
    """
    ...
    Example:
    >>> Get_All_Tradingday_1(engine,beginday='20220101',endday='20230101')
...
# 获取指数
def Get_index(engine, beginday, endday, stock_code:str):
    '''
...
# 获取某个行业的股票信息
def Get_stock_info(engine, beginday, endday, industry:str, data_type='Timeseries'):
    """
...
# 获取给定日期前或后多少个交易日
def cheak_day(engine,day,n,forward=False):
    if forward == False:
...
# 读取数据合集
def Get_data(engine, day, n, data_type,forward=False,
             industry=None,
             stock=None,
             Ashares=False):
    """
    ...
    Example 1:
    >>> Get_data(engine,endday='20220101',n=10,forward=False,industry='交通运输')

    返回: 一个交通运输行业的DataFrame

    Example 2:
    >>> Get_data(engine,endday='20220101',n=10,Ashares=True)
    ...
    assert (a + b + bool(Ashares)) == 1, "注意：在进行参数传递过程中，industry、stock和Ashares有且只能传入一个参数，其余两个无须进行传递，例如Get_data(engine,endday='20220101',train_len=15,iag_len=10,industry='交通运输')"
    ...
    if forward == False:
        ...
        # 如果行业非空，获取某个行业的股票信息
        if industry is not None:
            data = Get_stock_info(engine, start_day, end_day, industry)

        # 如果行业为空，股票代码不为空，则获取某个股票代码的信息
        if industry is None and stock is not None:
            data = Get_stock(engine, start_day, end_day, stock_code=stock)
        ...
        if Ashares:
            data = Get_stock_info(engine, start_day, end_day, industry=None, data_type=data_type)  #Get_stock_info这个函数如果未指定industry的话则代表所有行业
        return data
        ...
        # 如果行业非空，获取某个行业的股票信息
        if industry is not None:
            data = Get_stock_info(engine, day, end_day, industry)

        # 如果行业为空，股票代码不为空，则获取某个股票代码的信息
        if industry is None and stock is not None:
            data = Get_stock(engine, day, end_day, stock_code=stock)
        ...
        # 如果Ashares打开则代表所有行业的信息
        if Ashares:
            data = Get_stock_info(engine, day, end_day, industry=None, data_type=data_type)  #Get_stock_info这个函数如果未指定industry的话则代表所有行业
        return data
